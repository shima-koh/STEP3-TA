...
# 2. 入力用コード  
st.sidebar.write('<h3>■ ご開業予定のサロンについて、下記データを参考に教えてください。</h3>', unsafe_allow_html=True)
st.sidebar.write('データ① : サロンの1日の平均来客数 from 厚生労働省')
image1 = Image.open("image/美容室の平均客数.png")
st.sidebar.image(image1)

st.sidebar.write('データ② : 開業サロンのカット席数と店舗面積 from 開業サロン.com ')
image2 = Image.open("image/サロン開業実績_席数と店舗面積.png")
st.sidebar.image(image2)

st.sidebar.write('データ③: サロンイメージと内装費用の目安')
...
operationg_ratio = st.sidebar.slider('質問5. ご開業されるサロンの稼働率を教えて下さい。※数値表示。60%の場合は0.6を選択)', 0.0, 1.0, 0.6, 0.01)

st.sidebar.write('データ④ : スタイリストの平均年収')
...
st.sidebar.write('データ⑥ : 男性客が1回の来店で使う金額')
image6 = Image.open("image/男性がサロンで使った金額.png")
st.sidebar.image(image6)

cut_price = st.sidebar.slider('質問7. ご開業されるサロンの想定客単価(円)を選択して下さい。', 0, 20000, 8000, 100)/10000

st.sidebar.write('') 
st.sidebar.write('<h3>■ サロンをご開業されるエリアを教えてください。</h3>', unsafe_allow_html=True)
st.sidebar.write('データ⑦ : 首都圏駅利用者数ランキング')
image7 = Image.open("image/首都圏駅_利用者数ランキング.png")
st.sidebar.image(image7)
station = st.sidebar.selectbox('質問8. どの駅の近くでご開業予定か教えて下さい。', station_list)
line = st.sidebar.selectbox('質問9. その駅の路線を教えて下さい。', line_list)

st.sidebar.write('ご回答ありがとうございました。')
st.sidebar.write('検索実行ボタンを押してください。')
st.sidebar.write('') 
