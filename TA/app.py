...
image1 = Image.open("image\内装イメージ.jpg")
st.image(image1)

interior_cost = st.slider('Q1. サロンの内装費用（想定坪単価）を選択して下さい。', 0, 10, 30)

st.write('下のグラフは 席数vs店舗面積 の開業実績です。')
image2 = Image.open("image\サロン開業実績_席数と店舗面積.png")
st.image(image2)

seat_count = st.slider('Q2. サロンのシート数を選択して下さい。', 1, 2, 4, 8)
stylist_count = st.slider('Q3. 雇用するスタイリストの人数を選択して下さい。', 1, 10, 1, 1)
stylist_salary = st.slider('Q4. スタイリストの平均給与を選択して下さい。', 0, 10, 25, 1)
turnover = st.slider('Q5. シート回転数/日を選択して下さい。', 1, 12, 6, 1)
operationg_ratio = st.slider('Q6. 目標シート稼働率を選択して下さい。', 0.0, 1.0, 0.6, 0.01)
cut_price = st.slider('Q7. 想定の客単価を選択して下さい。', 0, 20000, 8000, 100)
st.write('') 
st.write('下のグラフは 首都圏駅の利用者数ランキングです。')
image3 = Image.open("image\首都圏駅_利用者数ランキング.png")
st.image(image3)
station = st.selectbox('Q8. どの駅の近くで開業するか選択して下さい。', station_list)
line = st.selectbox('Q9. 駅の路線を選択して下さい。', line_list)

st.write(f'まずは「 {line}{station} 」の周辺情報を調査します。')
st.write('') 
...
def get_area_info(line, station):
    if station_data:
        ...
        df6['カット単価(円~)'] = df6['カット単価(円~)'].str.replace(',', '').astype(float) #データ数値化
...
if response.status_code == 200:
        ...
        if tenanto_info is not None:
            # リクエストが成功した場合の処理
            st.write(' ・空きテナント検索数:', tenanto_info['空テナント数'], '件')
            # SQLite接続
            conn = sqlite3.connect('STEP3チーム課題_TA_IndéMode_DB.db') 
            # SQLクエリを実行してデータフレームに読み込む
            ...
            st.write(f' ・開業リスクの低いテナント上位5件  ※開業リスク= 損益分岐点/想定売上')
            st.write(f'   リスク1.0以上 の物件は収支赤字が見込まれる物件でおススメできません。必要に応じて開業条件を見直して下さい。')
            st.write(df8.head())
            # おススメ物件を地図に表示
